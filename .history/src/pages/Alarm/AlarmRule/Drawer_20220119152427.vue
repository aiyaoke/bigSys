...
import { deepCopy, showMessage } from "@/common/utils";
export default {
  props: {
    flag: {
      type: Boolean,
  ...
  data() {
    return {
      ...
      rules: {
        almTypeId: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
        initNum: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
        maxNum: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
        pushTypeIds: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
        userIds: [{ required: true, message: "请输入活动名称", trigger: "blur" }],
      }
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      let reqData = deepCopy(this.formData),
        { id, pushTypeIds, status } = reqData,
        fn = id ? apiUpdateAlarmRule : apiAddAlarmRule;
      reqData.status = +status;
      reqData.pushTypeIds = pushTypeIds.map(
        item =>
          ({
            邮件: 1,
            微信: 3

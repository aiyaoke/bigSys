...
import { apiCheckIsExpired } from "@/api/user";
...
export default {
  ...
  methods: {
    ...
    async getVideo() {
      let { data } = await apiGetVideo({ dtuId: this.currentDevice.id });
      if (!data.length)
        return showMessage("error", this.$translate("暂未接入摄像头"));
    ...
    async checkIsExpired() {
      let res = await apiCheckIsExpired({
        equipment: this.currentDevice.id
      });
      if (res.code === false) {
        this.getVideo();
      } else {
        showMessage("error", "监控过期，请先充值");
      }
    }
  },
  beforeDestroy() {
